...
export function ItineraryRow({ item, isLast }: ItineraryRowProps) {
  ...
  return (
    <div ref={ref} className={`reveal mb-1.5 flex items-start ${isVisible ? 'reveal-visible' : ''}`}>
      ...
      <div className="relative flex w-5 flex-shrink-0 justify-center">
        ...
        {!isLast && (
          <span
            aria-hidden="true"
            className="absolute left-1/2 top-[17px] bottom-[-22px] w-px -translate-x-1/2 bg-[repeating-linear-gradient(180deg,#A9863E_0px_5px,transparent_5px_9px)]"
          />
        )}
      </div>
      <div className="pb-[22px] pl-4 pt-0.5 font-garamond text-[17px] italic text-ink">{item.event}</div>
    </div>
  );
}
